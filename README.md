...
Optional, but a strong way to stand out. Pick **one** (don't do both — we'd
rather see the core done well):

- **CSV export** — export all leads from the dashboard as CSV, generated by the
  backend (bonus if it streams rather than buffering everything in memory).
- **Webhook endpoint** — a public API endpoint that ingests leads from an
  external source (e.g. `POST /api/webhooks/leads`), rate-limited via Valkey.
