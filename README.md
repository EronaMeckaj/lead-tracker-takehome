...
We respect your time: aim for **8–10 focused hours**. If you're going well past
...
### 6. CSV export
- Export all leads from the dashboard as CSV, generated by the backend
  (bonus if it streams rather than buffering everything in memory).

### 7. Webhook endpoint
- A public API endpoint that ingests leads from an external source
  (e.g. `POST /api/webhooks/leads`), rate-limited via Valkey.
